...
import de.htwberlin.kbe.gruppe7.MauMauSpiel.ui.view.View;
...
public class UIContollerImpl implements UIControllerService {
    ...
    public UIContollerImpl (SpielService spielService, SpielerService spielerService, KartenService kartenService,
                            KIService kiService, View view){
    ...
    private void spielablauf( Spiel spielinstanz)  {
        ...
        while(spielAktiv && !spielWurdeAbgebrochen){
            ...
            if(!spielWurdeAbgebrochen) {
                ...
                if (!spielAktiv) {
                    view.gewonnen(spielinstanz.getAktiverSpieler().getName());
        ...
        while (!spielWurdeAbgebrochen) {
            try {
                ...
                if (eingabe == 100) {
                    ...
                    mauMauGesagt = true;
                    view.karteLegenNachMau();
                } else if (eingabe == 300) {
                    spielWurdeAbgebrochen = true;
                    view.spielWirdAbgebrochen(spielID);
                } else {
                    view.karteExistiertNicht();
                }
            }
        }
        ...
        if (!spielWurdeAbgebrochen) {
            if (mauMauGesagt) {
                if (spielService.pruefeAufMau(spiel.getAktiverSpieler())) {
                    view.spielerMauSagen(spiel.getAktiverSpieler().getName());
                } else {
                    spielService.kartenAusZiehstapelZiehen(1, spiel);
                    view.spielerFalschMauGesagt(spiel.getAktiverSpieler().getName());
                }
            }

            if (karteZiehen) {
                spielService.kartenAusZiehstapelZiehen(1, spiel);
                view.karteGezogen(spiel.getAktiverSpieler().getName());
            } else if (spielService.getRegelnwerl().prüfeZweiKartenZiehen(gelegteKarte)) {
                spielService.kartenAusZiehstapelZiehen(2, spiel);
                view.mussKartenZiehen(spielerService.getNextSpieler(spiel.getSpieler(), spiel.getAktiverSpieler()).getName(), spiel.getAnzahlKartenZiehen(), "");
            } else if (spielService.getRegelnwerl().spielerAussetzen(gelegteKarte)) {
                spiel.setNaechsterSpielerMussAussetzen(true);
                view.mussAussetzen(spielerService.getNextSpieler(spiel.getSpieler(), spiel.getAktiverSpieler()).getName());
            } else if (spielService.getRegelnwerl().prüfeZweiKartenZiehen(gelegteKarte) ) {
                spiel.setNaechsterSpielerMussFarbeWuenschen(true);
                neueSpielfarbeSetzen();
            } else if (spielService.getRegelnwerl().prüfeGelegteKarteGueltig(gelegteKarte, spiel.getObersteKarteAblagestapel(), spiel.getSpielfarbe())) {
            ...
            if (spielService.pruefeAufMau(spiel.getAktiverSpieler())) {
                spielService.kartenAusZiehstapelZiehen(1, spiel);
                view.mussKartenZiehen(spiel.getAktiverSpieler().getName(), 1, " da er nicht 'Mau' gesagt hat");
            }
    ...
    private void virtuellerKISpielzug() {
        ...
        view.kiSpielt(spiel.getObersteKarteAblagestapel().toString(),spiel.getAktiverSpieler().getName());
        ...
        do{
            ...
            if (kartenIndex > spiel.getAktiverSpieler().getSpielerStapel().size()){
                ...
                view.kiHatKarteGezogen(spiel.getAktiverSpieler().getName());
                erfolgreichGelegtOderGezogen = true;
            }
        ...
        view.kiHatGespielt(gelegteKarte.toString(),spiel.getAktiverSpieler().getName());
    ...
    private void informationenSpieler(Spieler spieler, String obersteKarte){
        ...
        view.kartenInfosAnzeigen(obersteKarte, kartenStrings, spieler.getName());
    }
    ...
    private void neueSpielfarbeSetzen() {
        ...
        view.neueSpielfarbeInfo(spiel.getAktiverSpieler().getName(), neueSpielfarbe);
    }
    ...
    public int hauptmenuKontroller(int eingabe)  {
        ...
        switch (eingabe) {
            case 1:
                ...
                view.spielenOderRegelnAnzeigen();
    ...
    private Spiel neuesSpielErzeugen() {
        ...
        for (int i = 0; i<= anzahlSpieler-1; i++){
            view.nameNeuerSpielerMitIndex(i+1);
        ...
        spielService.getRegelnwerl();
        spiel.setId(spielID);
        view.anzeigeSpielID(spielID.toString());
